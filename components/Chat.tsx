...
type ChatProps = {
  chatInfo:
    ...
    | { directChatId: string; username: string; type: "direct_chat" };
};

async function Chat({ chatInfo }: ChatProps) {
  const chatId = chatInfo.type === "server_chat" ? chatInfo.channel.id : chatInfo.directChatId;
  const chatName = chatInfo.type === "server_chat" ? chatInfo.channel.name : chatInfo.username;
  ...
  return (
    <div className="h-[calc(100%_-_48px)]">
      <Suspense fallback={<ChatLoadingSkeleton />}>
        <MessagesList chatType={chatInfo.type} chatId={chatId} chatName={chatName} />
      </Suspense>
